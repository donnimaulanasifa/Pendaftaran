<template>
  <a role="button" @click.prevent="switchTheme()" :aria-label="'Switch to ' + nextTheme + ' mode'" class="nav-link">
    <span key="dark" v-if="theme === 'dark'">Ubah ke mode terang</span>
    <span key="light" v-else-if="theme === 'light'">Ubah ke mode gelap</span>
    <span key="light" v-else>Ubah tampilan</span>
  </a>
</template>
